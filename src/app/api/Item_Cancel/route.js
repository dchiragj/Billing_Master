  ...
  try {
    ...
    if (!iCode || !entryBy) {
      return NextResponse.json(
        { status: false, message: "iCode and entryBy query parameters are required" },
        { status: 400 }
      );
    }
    ...
    request.input("ICode", sql.VarChar(50), iCode);
    request.input("entryby", sql.VarChar(50), entryBy);

    console.log("Executing Stored Procedure: USP_Item_Cancel with params:", {
      iCode,
      entryBy
    });
    const result = await request.execute("USP_Item_Cancel");
