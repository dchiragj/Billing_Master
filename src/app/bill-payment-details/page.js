...
//   const isCashSelected = formData.MRHDR.paymode === "190"; 
...
//               ].map(([name, label, type, disabled, placeHolder], index) => (
...
//             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
//               {[
//                 ["paymode", "Receipt Mode", "select", true, ["cash", "Account"]],
...
//           ["NETAMT", "Amount Applicable", "number", true],
//           ["MRSCASH", "Cash Amount", "number", true],
//           ["MRSCHQNO", "Cheque Number", "text", true],
//           ["MRSCHQDT", "Cheque Date", "date", true],
//           ["MRSCHQ", "Cheque Amount", "number", true],
//           ["OnAccountBalance", "On Account Balance", "number", true],
//           ["MRSCHQ", "Collection Amt. From Cheque", "number", true],
//           ["DED_TDS", "TDS Type", "text", false],
//           ["MRSBANK", "Direct Deposited in Bank", "select", true, ["Y", "N"]],
//           ["MRSBANK", "Deposited Bank", "text", false],
//           ["OnAccountYesNo", "On Account Yes/No", "checkbox", false],
//           ["NETAMT", "NET RECEIVED", "number", true],
//         ].map(([name, label, type, isRequired, options], index) => (
//           <div key={index} className="flex items-center">
//             <label className="text-gray-700 font-medium w-1/3 text-left">{label}</label>
//             {type === "select" ? (
...
//                 onChange={(e) => handleInputChange(e, "MRHDR")}
//                 className={`p-2 w-2/3 bg-gray-100 rounded-md border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:outline-none ${
//                   (name === "MRSCASH" && !isCashSelected) || (name !== "MRSCASH" && isCashSelected) ? "opacity-50 cursor-not-allowed" : ""
//                 }`}
//                 required={isRequired}
//                 disabled={(name === "MRSCASH" && !isCashSelected) || (name !== "MRSCASH" && isCashSelected)}
//               />
//             )}
//           </div>
//         ))}
...
const BillPaymentDetails = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Filter and include only the fields present in the form data
    ...
    <div className="p-8 w-full lg:w-[calc(100vw-288px)] ml-0 lg:ml-[288px] text-black min-h-screen">
      ...
      <div className="bg-white p-8 rounded-lg shadow-lg space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
          <div className="border rounded-lg p-5">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[
                ...
                ["Remarks", "Remarks", "textarea", false],
              ].map(([name, label, type, disabled, placeHolder,options], index) => (
                <div key={index} className={`flex items-center ${type === "textarea" ? "md:col-span-2" : ""}`}>
                  <label className={`text-gray-700 font-medium ${label.includes("Remarks") ? "lg:w-1/6" : ""} w-1/3 text-left`}>{label}</label>
                  {type === "select" ? (
                    <select
          ...
          <div className="border rounded-lg p-5">
            <h5 className="text-lg font-semibold mb-4 flex justify-center">Cheque Details</h5>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[
          ...
          <div className="border rounded-lg p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[
                ...
                ["NETAMT", "Amount Applicable", "number", true],
                ["MRSCASH", "Cash Amount", "number", true],
                ["MRSBR", "MRSBR", "text", true],
                ["MRSDT", "Cheque Date", "date", true],
                ["MRSTYPE", "Cheque Amount", "text", true],
                ["EMRS_type", "EMRS_type", "text", true],
                ["BankAcccode", "BankAcccode", "text", true],
                // ["DED_TDS", "TDS Type", "text", false],
                // ["MRSBANK", "Direct Deposited in Bank", "select", true, ["Y", "N"]],
                // ["MRSBANK", "Deposited Bank", "text", false],
                // ["OnAccountYesNo", "On Account Yes/No", "checkbox", false],
