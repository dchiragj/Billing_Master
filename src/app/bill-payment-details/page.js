...
//   const billsParam = searchParams.get("selectedBill");
//   const companyCode = searchParams.get("companyCode");
//   const partyCode = searchParams.get("partyCode");

//     const handleSelectedBillsForms = async () => {
//     try {
//       const data = await getBillEntryPayment(billsParam, userDetail.CompanyCode);
...
//       // }
//     } catch (err) {
//       console.log(err.response?.data?.message || "An error occurred while fetching data");
//     } 
//     console.log("Selected Bill Nos:", billsParam);
//   };

//   const selectedBills = billsParam ? billsParam.split(",") : [];
...
const BillPaymentDetails = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const response = await billgenerate({...formData, CompanyCode:"1",EntryBy: userDetail.UserId, FinYear:Finyear, MR_Location :"1"})
    console.log(response);
    
    // console.log({...formData, CompanyCode:"1",EntryBy: userDetail.UserId, FinYear:Finyear, MR_Location :"1"}); 
  };
  ...
  <div className="border-2 border-gray-300 p-4 rounded-lg">
    ...
    {formData.BillMRDET.BillMRDET.map((bill, index) => (
      <div key={index} className="mb-6">
        <h6 className="text-md font-semibold mb-2">Bill {index + 1}</h6>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[
