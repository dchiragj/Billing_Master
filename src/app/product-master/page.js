  ...
  const ProductMaster = () => {
    ...
    const handleInputChange = (e, section = "IMst", index = 0) => {
      ...
      if (type === "file" && files?.length > 0) {
        ...
        reader.onloadend = () => {
          ...
          setFormData((prev) => ({
            ...prev,
            IMst: [{ ...prev.IMst[0], [name]: base64String }], // Store Base64
          }));
        };
    ...
    //   //   setImagePreview(URL.createObjectURL(files[0].name));
    ...
    const handleEditClick = async (product) => {
      ...
      try {
        ...
        if (data.itemDetails[0].Doc_Path) {
          setImagePreview(data.itemDetails[0].Doc_Path); // Set image preview if Doc_Path exists
        }
      } catch (error) {
                      ...
                      <tr className="border border-gray-300">
                        ...
                        <td className="border border-gray-300 px-4 py-2">
                          <input
                            type="file"
                            name="Doc_Path"
                            // value={formData.IMst[0].Doc_Path} // This is causing the error
                            onChange={(e) => handleInputChange(e)}
                            className="p-2 w-2/3 bg-gray-100 rounded-md border border-gray-300 focus:ring-2 focus:ring-gray-500 focus:outline-none"
                          />
                        </td>
                        <td className="border border-gray-300 px-4 py-2">
                          { console.log("Base64 String:", `data:image/jpeg;base64,${formData.IMst[0].Doc_Path}`)}
                          {imagePreview && (
                            <img
                            src={`data:image/jpeg;base64,${formData.IMst[0].Doc_Path}`} 
                          ...
                          {/* {formData.IMst[0].Doc_Path ? (
                            <img
                              src={URL.createObjectURL(formData.IMst[0].Doc_Path)}
                              alt="Uploaded Image"
