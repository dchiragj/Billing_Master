...
const CustomerMaster = () => {
  ...
  useEffect(() => {
    if (userDetail.CompanyCode) {
      setDropdownLoading(true); // start loading
      ...
      Promise.all(promises).then(() => {
        setDropdownLoading(false); // finish loading
      });
    }
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      const customerLocationIds = formData.CustomerLocationId.map(
        (option) => option.value
      )
        .join(",");
  ...
  const handleAddClick = () => {
    const defaultFormData = {
      ...
      BillType:
        dropdownData.BillType.length > 0
          ? dropdownData.BillType[0].DocCode
          : "",
      TaxType: dropdownData.TAX.length > 0 ? dropdownData.TAX[0].DocCode : "",
      PriceType:
        dropdownData.Price.length > 0 ? dropdownData.Price[0].DocCode : "",
  ...
  const handleEditClick = async (customerData) => {
    ...
    const updatedFormData = {
      ...
      State:
        customerData.State ||
        (dropdownData.State.length > 0 ? dropdownData.State[0].DocCode : ""),
    ...
    if (showActionButtons) {
      rowData["Action"] = (
        <div className="flex gap-3">
          ...
          <button
            onClick={() => handleDeleteClick(customerData.CustomerCode)}
          ...
          <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-6xl overflow-auto max-h-[90vh] border-2 border-gray-300">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl">
                {isEditMode ? "Edit Customer Master" : "Add Customer Master"}
              </h3>
              <button
            ...
            >
              ...
              <div className="pb-4">
                <div className="text-xl font-semibold flex justify-center pb-2">
                  Contact Details{" "}
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                  {[
                      ...
                      >
                        <label
                          className={`text-gray-700 font-medium ${
                            label.includes("Address") ? "lg:w-1/6" : ""
                          } w-1/3 text-left`}
                        >
                          {label}
                        </label>
