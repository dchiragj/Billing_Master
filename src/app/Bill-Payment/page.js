...
import { getBillEntryPayment, getBillPaymentData } from "@/lib/masterService";
...
const BillPaymentForm = () => {
  ...
  const handleNavigate = async () => {
    try {
      const data = await getBillEntryPayment(selectedBill,userDetail.CompanyCode);
      if (data) {
        setBillData(data);
      } else {
        setError("Failed to fetch data from the API");
    ...
    console.log("Selected Bill Nos:", selectedBill);
  };
            ...
            <div className="flex justify-center my-4">
              <button
                ...
                className={`px-6 py-2 rounded-lg transition duration-200 ${
                  selectedBill.length > 0 ? "bg-blue-600 text-white hover:bg-blue-700" : "bg-gray-300 text-gray-500 cursor-not-allowed"
                }`}
              >
